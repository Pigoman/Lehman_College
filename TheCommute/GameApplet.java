...
public abstract class GameApplet extends Applet implements KeyListener, Runnable
{
   ...
   // Boolean array containing state of keyboard
   ...
   public final void run()
   {
      ...
      while ( !finished )
      {
         ...
         //if(leftPressed)  Driver.player.rotateLeftBy(4);
         //if(rightPressed) Driver.player.rotateRightBy(4);
         //if(upPressed)    Driver.player.moveForwardBy(6);
         //if(downPressed)  Driver.player.moveBackwardBy(3);
      }
   ...
   public final void keyPressed(int code, boolean shiftPressed)
	{
		input[code] = true;
	}
